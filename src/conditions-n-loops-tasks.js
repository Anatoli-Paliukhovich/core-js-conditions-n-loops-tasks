...
function convertToRomanNumerals(num) {
  ...
  const ROMAN_DOZENS = ['', 'X', 'XX', 'XXX'];
  result += ROMAN_DOZENS[Math.floor(num / 10)];
  result += ROMAN_SINGLES[num % 10];
  return result;
}
...
function convertNumberToString(numberStr) {
  ...
  for (let i = 0; i < numberStr.length; i += 1) {
    result += numberToString(numberStr[i]);
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < result.length; i += 1) {
    for (let j = 0; j < Math.floor(result.length / 2); j += 1) {
      const temp = result[i][j];
...
function sortByAsc(arr) {
  ...
  for (let i = 0; i < result.length; i += 1) {
    ...
    for (let j = i + 1; j < result.length; j += 1) {
      if (result[j] < result[indMin]) {
        indMin = j;
      }
    }
    const temp = result[i];
    result[i] = result[indMin];
    result[indMin] = temp;
  }
  return result;
}
...
function getNearestBigger(number) {
  ...
  let smallNumInd = ind + 1;
  for (let i = ind + 2; i < nums.length; i += 1) {
    if (nums[i] > nums[ind] && nums[i] < nums[smallNumInd]) {
      smallNumInd = i;
    }
  }
